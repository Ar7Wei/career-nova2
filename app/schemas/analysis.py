...
它是改动记录（`change_records`，origin=job_analysis）里子项仍待定的一条。未处理 tab 的接口
把两者一起返回（一份报告 + 一列待收录的处方），是"一屏两物"而不是"一物两存"。
...
class AnalysisReportOut(BaseModel):
    ...
    待收录处方（点「改进」→ 子项 pending 转 confirmed）。
    """
    ...
    created_at: str = ""
    suggestions: list[ChangeRecord] = Field(default_factory=list)
