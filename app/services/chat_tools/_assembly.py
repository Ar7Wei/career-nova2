...
# 本轮 agent 开跑的时间点快照（2026-09-14，apply.md §11.7.8）。`render_records_panel` 的
# 「← 你刚改的」靠它比对：`updated_at > _panel_changed_since` 的记录 = 本轮被动过。
# 单用户单在途对话，一个模块级变量即可（与 chat.py 的 _current_cancel_event 同型）。
# begin_turn() 在每轮跑 agent 前记一次；None 时不标任何行。
_panel_changed_since: datetime | None = None


def begin_turn() -> None:
    """记下本轮开跑时间点（每轮跑 agent 前调一次）。据此标「刚改的」记录。"""
...
async def build_deep_system_prompt() -> str:
    """Deep agent 每轮 system prompt：读 facts/持久决策/简历/改动记录 → 拼瘦身版 base prompt。
    ...
    并复用 `optimization.render_records_panel` 渲染改动记录（含变化标记，不重复造）。
    """
    facts = await list_facts(status="active")
    # 嵌套展平：条目一行 + 各要点缩进行（无 points 只有条目行）——共用 utils 的展平口径。
    # with_id：对话 agent 要能指认某条事实（set_fact_on_resume 翻「不显示」得给 id）。
    facts_text = flatten_facts(facts, bullet_title=True, with_id=True)
    resume = await latest_document()
    prefs_text = await build_decisions_text()
    # changed_since = 本轮开跑时间点（begin_turn 记的）：标出本轮 agent 自己动过 / 用户在
    # 面板上操作过的行，免得 agent 把动作误当"一开始就在这儿"。
    panel_text = await render_records_panel(changed_since=_panel_changed_since)
