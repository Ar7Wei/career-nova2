...
@tool(
    "query_market",
    description=(
        "现场查招聘市场（猎聘），拿客观统计，帮你判断方向对不对、用户缺什么（ADR 0007）。\n\n"
        "定方向前摸底、或想验证「这个方向机会多不多、门槛在哪、该去哪个城市」时调用。"
        ...
        "**用它验自己的词是否真在收窄**：平台对多词是「有交集按 AND、交集不足静默放宽成 OR」"
        "（ADR 0018）。若「全命中」≈ 最大单词命中 → AND 没生效、这组在按单词搜，该换收窄词或拆组。"
        "诊断只给计数，**判断归你**。\n"
        "- 只返回客观数字，**不做判断**——「用户 vs 市场缺什么」由你拿这些数字比对已注入的"
        "facts（经历/技能/学历）后自己讲出，带理由、带差距、带风险。\n"
        "- 样本**仅猎聘**（BOSS/前程无忧/智联没纳入），讲的时候诚实标注，别装全市场。\n"
