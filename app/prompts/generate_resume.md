...
# 用户的持久决策（可选）
下面是用户的**跨版本决策记录**（拍过板的改动原则），生成时全程遵守。它可能要求**板块归并**：
- 例如「不单独开项目经历栏，内容并进工作经历」——这时把 `projects` 类事实**折进 `work`**（或别的板块），`layout` 不再单列 `projects`。
- **归并不是屏蔽**：被归并的事实**仍须覆盖**——其关键信息（公司/项目名/技术词）要出现在折入的板块里，不能丢。只是不单独开一个板块标题。
