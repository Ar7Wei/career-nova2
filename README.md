...
它不替你投简历，也不替你决定去哪家。**筛岗位、投简历、做决定的始终是你**——它只是把机械的部分接过去。
...
| **投递** | ✅ 已完成 | 岗位聚合（猎聘 + 前程无忧，跨平台统一列表）、去重、排序；投递跟进状态线；面试日历 |
| **市场调研** | 🟡 部分 | 简历 agent 的 `query_market` 工具：现场查岗位量 / 城市 / 学历 / 年限 / 薪资分布。**「哪个方向在涨」这类广度探测尚未做** |
| **投递自动化** | ⬜ 未开始 | 预填、自动投递、自动抓结果——v1 是手动挡，自动化留作渐进升级线头 |
| **Chatter**（行业文档 RAG） | ⬜ 未开始 | 基于 sqlite-vec 的行业文档问答 |
| **Growth** | ⬜ 未开始 | — |
...
| 层 | 选型 |
|---|---|
| 壳 | Electron 35 — 托盘驻留、子进程管理（`taskkill /F /T` 整树杀 + 端口核验）、招聘平台 DOM 抓取、`printToPDF` 导出 |
...
- [x] 岗位聚合（猎聘 + 前程无忧）
- [x] 投递跟进状态线 + 面试日历
- [x] 打包分发（免装 Python / uv 的安装包，NSIS + Portable）
- [ ] 投递执行自动化（预填 / 自动投递 / 自动抓结果）
- [ ] Chatter — 行业文档 RAG
- [ ] 市场广度探测（"哪个方向在涨"）
- [ ] Growth
